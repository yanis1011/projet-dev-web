var userLang = navigator.language || navigator.userLanguage;
  ...
  `;
    ...
    function selectAnswer(qId, aId) {
      selectedAnswers.push(`Q${qId}_A${aId}`);
    
      if (selectedAnswers.length === forms.length) {
        const answerString = selectedAnswers.join('...') + '...';
        const targetFile = `${answerString}.fr.html`;
        if (targetFile === "Q1_A1...Q2_A3....fr.html") {
          window.location.href = targetFile;
        } else {
          alert("❌ Mauvaises réponses. Redirection vers le CV.");
    ...
    async function brutforce() {
      ...
      alert("Toutes les combinaisons ont été ouvertes dans de nouveaux onglets. L'un des onglets contient la bonne réponse. Cette limitation est due au fait que cette page est accédée localement et non servie par un serveur.");
    }
